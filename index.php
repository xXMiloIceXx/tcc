...
if (isset($_GET['del'])) {
    $id = $_GET['id'];
    $q2 = mysqli_query($conn, "update users set deleted_at='" . date('Y-m-d H:i:s') . "' where id='$id'");
    if ($q2) {
...
<body>
    <div class="container my-5">
        <div class="row py-4">
            ...
            <div class="col-md-6 col-12"><a class="btn btn-primary float-end" href="create.php">Add User</a></div>
        </div>
        <table class="table table-striped table-bordered datatable border">
            ...
            <tbody>
                <?php
                $q1 = mysqli_query($conn, "select * from users where deleted_at is null");
                while ($d1 = mysqli_fetch_assoc($q1)) {
                ?>
                    <tr>
                        <td><?= $d1['id'] ?? '0' ?></td>
                        <td><?= $d1['username'] ?? 'Unknown' ?></td>
                        <td><?= $d1['email'] ?? 'No email' ?></td>
                        <td><?= $d1['role'] ?? 'No role' ?></td>
                        <td><?= $d1['status'] ?? 'Inactive' ?></td>
